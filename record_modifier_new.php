...
<? 
	...
	if(is_array($arResult["RECORD_LEGEND"])):
		if(isset($arResult["FIELD_CELLS"]["START"]["ACTUAL_INFO"])):
		unset($arResult["FIELD_CELLS"]["START"]["ACTUAL_INFO"]);
		endif;
          

		if($arResult["RECORD_LEGEND"][count($arResult["RECORD_LEGEND"])-1]['TIME_LEAKS_OVERALL'] != '00:00'):
			$arResult["FIELD_CELLS"]["BREAK"]["HIDE"] = null;
		endif;
		$arResult["FIELD_CELLS"]["START"]["ACTUAL_VALUE"] = $arResult["RECORD_LEGEND"][0]["TIME_START"];
	$arResult["FIELD_CELLS"]["START"]["RECORDED_VALUE"] = $arResult["RECORD_LEGEND"][0]["TIME_START"];
	...
	$arResult["FIELD_CELLS"]["DURATION"]["RECORDED_VALUE"] = $arResult["RECORD_LEGEND"][count($arResult["RECORD_LEGEND"])-1]['TIME_DURATION_OVERALL'];
	$arResult["FIELD_CELLS"]["DURATION"]["ACTUAL_VALUE"] = $arResult["RECORD_LEGEND"][count($arResult["RECORD_LEGEND"])-1]['TIME_DURATION_OVERALL'];
	endif;
	//echo '<pre>';
 //print_r($arResult["RECORD_LEGEND"]); ?>
	...
	//endif; ?>
		...
		<? $num = 0; ?>
				...
				$stages = ["Вход", "Выход", "Продолжительность"];
			endif;?>
						<div class="timeman-report-time-list">
					<?	foreach($stages as $stage): ?> <?//print_r($recordLegend); ?>
						<? if($stage == 'Вход'):
							$timeVar = $recordLegend['TIME_START'];
							$cssClasses = 'timeman-report-time-item-data-small';
							elseif($stage == 'Перерыв' && !empty($recordLegend['TIME_LEAKS'])):
								...
								elseif($stage == 'Продолжительность' && !empty($recordLegend['DURATION'])):
									$timeVar = $recordLegend['DURATION'];	
									//$cssClasses = 'timeman-report-time-item-leaks';
									elseif($stage == 'Выход' && !empty($recordLegend['TIME_FINISH'])):
										$timeVar = $recordLegend['TIME_FINISH'];	
										$cssClasses = 'timeman-report-time-item-changed-small';
										elseif($stage == 'Выход' && empty($recordLegend['TIME_FINISH'])):
											$timeVar = $recordLegend['TIME_FINISH'];	
				...
				>
						<div class="timeman-report-time-item-title">
						<span class="timeman-report-time-item-title-text-small"><? if($cssClasses != 'timeman-report-time-item-blank'): echo htmlspecialcharsbx($stage); endif; ?></span>
					</div>
					<div class="timeman-report-time-item-inner-small">
						<div class="timeman-report-time-item-data">
							<span class="timeman-report-time-item-value-small"
...
<? if($USER->GetId() == 3071):
	//echo '<pre>';
	//print_r($arResult['RECORD_LEGEND']);
	if(isset($recordLegend['TIME_FINISH'])):?>
	<? $stages = ["Выход", "Вход", "Перерыв"]; ?>

	<div class="timeman-report-time-list">
<? 
		$timeVarPlus = $arResult["RECORD_LEGEND"][$num+1]["TIME_START"];
